...
//  printf("qold[0] = %f, qold[1] = %f, qold[2] = %f, qold[3] = %f\n", qold[0], qold[1], qold[2], qold[3]);
//  printf("q[0] = %f, q[1] = %f, q[2] = %f, q[3] = %f\n", q[0], q[1], q[2], q[3]);
//  printf("res[0] = %f, res[1] = %f, res[2] = %f, res[3] = %f\n", res[0], res[1], res[2], res[3]);
//  printf("adt = %f, rms = %f\n", adt, rms);
  adti = 1.0f/(*adt);
...
//  printf("OP_WARPSIZE %d, tid %d\n", OP_WARPSIZE, tid);  
  ...
  int thread_divergance_offset = set_size % OP_WARPSIZE;
//  printf("thread_divergance_offset %d\n", thread_divergance_offset);
//  printf("set_size %d\n", set_size);
  for (int n=get_global_id(0); n<set_size+thread_divergance_offset; n+=get_global_size(0)) {
...
//    barrier( CLK_LOCAL_MEM_FENCE );
    // copy data into shared memory, then into local
//    if( n == 101248 || n == 101249 || n == 101250 || n == 101251) {
//      printf("iter n = %d step 1\n", n);
//    }
    if (nelems ==  OP_WARPSIZE || (nelems < OP_WARPSIZE && tid <  thread_divergance_offset)) {
      for (int m=0; m<4; m++)
...
//    if( n == 101248 || n == 101249 || n == 101250 || n == 101251) {
//      printf("iter n = %d step 2\n", n);
//    }

    //orig attempt if cond: n < set_size
    if  (nelems == OP_WARPSIZE || (nelems < OP_WARPSIZE && tid < thread_divergance_offset)) {
      for (int m=0; m<4; m++)
...
*/
  ...
  p1   = g_const_d->gm1*(q1[3]-0.5f*ri*(q1[1]*q1[1]+q1[2]*q1[2]));
  vol1 =  ri*(q1[1]*dy - q1[2]*dx);
  //printf("dx = %f, dy = %f, ri = %f, p1 = %f, vol1 = %f\n", dx, dy, ri, p1, vol1);
  ri   = 1.0f/q2[0];
  p2   = g_const_d->gm1*(q2[3]-0.5f*ri*(q2[1]*q2[1]+q2[2]*q2[2]));
  vol2 =  ri*(q2[1]*dy - q2[2]*dx);

  //printf("ri = %f, p2 = %f, vol2 = %f", ri, p2, vol2);
