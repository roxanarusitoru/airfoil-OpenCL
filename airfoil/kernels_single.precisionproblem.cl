...
(*/
  ...
  ri = (float) ((double)1.0f/(double)q1[0]);
  ...
  if (*bound==1) {
    ...
    ri   = (float) ((double) 1.0f/(double)g_const_d->qinf[0]);
...
//  printf("adt = %f, rms = %f\n", adt, rms);
  adti = (float) ((double)1.0f/(double)(*adt));
...
inline void adt_calc(__local float *x1,__local float *x2,__local float *x3,__local float *x4,__global float *q,__global float *adt, __constant struct global_constants *g_const_d){
  ...
  ri =  (float) ((double) 1.0f/(double)q[0]);
  u  =   ri*q[1];
  v  =   ri*q[2];
  c  = (float) sqrt((double) (g_const_d->gam*g_const_d->gm1*(ri*q[3]-0.5f*(u*u+v*v))));

  //printf("ri = %f, u = %f, v = %f, c = %f\n", ri, u, v, c);

  dx = x2[0] - x1[0];
  dy = x2[1] - x1[1];
  *adt  = fabs(u*dy-v*dx) + c*(float)sqrt((double) (dx*dx+dy*dy));
  ...
  *adt = (float) ((double)(*adt) / (double) g_const_d->cfl);
...
*/
  ...
  ri   = (float) ((double)1.0f/(double)q2[0]);
